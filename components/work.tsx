...
export const WorkBackground: React.FC = () => {
  return <div className="sticky top-0 grid w-full min-h-screen grid-cols-1 lg:grid-cols-2">
...
export const WorkLeft: React.FC<{progress: number, children: React.ReactNode}> = ({children, progress}) => {
  ...
  return <div className="flex flex-col items-center justify-center text-3xl lg:text-3xl h-[30vh] lg:h-auto"
    style={{
...
export const WorkRight: React.FC<{progress: number, children: React.ReactNode}> = ({children, progress}) => {
  ...
  // if (progress > 0.85) translateY = Math.max(-50, -(progress - 0.85) * 50 * 2);
  return <div className="flex justify-center flex-1 h-screen lg:items-center"
    style={{
      transform: `translateY(${translateY}px)`  
    }}
  >
